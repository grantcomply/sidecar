...
- ``FilterDropdown`` — a pill button (grey = no filter, blue = narrowing) whose
  checklist lives in a floating overlay. No "Deselect All" (Finding 2 fix).
...
class FilterDropdown(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._select_all_btn = ctk.CTkButton(
            self._overlay, text="Select all", height=24,
            font=ctk.CTkFont(size=11),
            fg_color=NEUTRAL_PILL, hover_color=NEUTRAL_HOVER, text_color=PILL_TEXT,
            command=self._select_all,
        )
        self._select_all_btn.grid(row=0, column=0, sticky="ew", padx=8, pady=(8, 4))
    ...
    def _update_label(self):
        ...
        if total == 0 or selected == total:
            ...
            only = next(n for n, v in self._vars.items() if v.get())
            text = f"{self._label}: {truncate(only, 10)}"
            fg = ACCENT_BLUE
        else:
            text = f"{self._label}: {selected}/{total}"
            fg = ACCENT_BLUE
...
class KeyOffsetControl(ctk.CTkFrame):
    ...
    def _refresh(self) -> None:
        ...
        self.configure(fg_color=ACCENT_BLUE if self._offset != 0 else NEUTRAL_PILL)
...
class DateRangeControl(ctk.CTkFrame):
    ...
    def _build_overlay(self, host: tk.Widget) -> None:
        ...
        ctk.CTkButton(
            manual, text="Apply", width=60, height=28,
            font=ctk.CTkFont(size=11),
    ...
    def _update_pill(
        ...
        fg = NEUTRAL_PILL if self._range == (None, None) else ACCENT_BLUE
...
class FilterBar(ctk.CTkFrame):
    ...
    resets all four controls and fires a single ``on_filter_change``.
    """
